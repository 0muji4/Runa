...
- IMPORTANT: git / gh の**状態変更（branch 作成・commit・push・PR/Issue 作成・merge）は実行しない**。Codex は調査・不整合の修正・**正確なコマンド列の出力**までを担い、実行はユーザーが行う。`git status` / `diff` / `log` などの読み取りは可。ただし、ユーザーから明示的に宣言があった場合はその限りではない。
