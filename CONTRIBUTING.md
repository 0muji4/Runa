...
`Server`（Go）の実コードを最初に入れる PR で、`apps/go/**` を対象にした CI（`go vet` / `go build` / `go test`）を `.github/workflows/` に追加する。コードが無い段階で先に置くと、`go.mod` を参照する CI が初回 PR で失敗するため、コードと同じ PR で導入する。
