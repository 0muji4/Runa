...
このリポジトリはバックエンド・共有ロジック・各 OS の UI を貫く縦の配線から始まり、その骨組みの上にプロダクト機能を**縦切り（vertical slice）**で 1 本ずつ載せてきた。実装済みの機能は本 README の以降の各節に記す。新しい機能も同じ型で足す。機能の足し方は [docs/adding-a-feature.md](docs/adding-a-feature.md) を参照する。
...
- **対象画面**: 07 きょうの一曲 / 08 これまでの一曲 / 09 ダイアリー / 11 ダイアリー詳細 / 12 ふりかえりカレンダー（戻るのみ。月ナビはコンテンツ）/ 日別記録 / 13 ギャラリー / 15 今日の月 / 16 インサイト / 19 設定 / 20 テーマ / 21 通知設定 / 22 プライバシー・ロック / 23 アカウント・データ。**対象外**は 06 ホーム（見出しなし）、10 ダイアリー作成、14 画像詳細、01–05 の認証フロー、ロック画面、状態画面。
