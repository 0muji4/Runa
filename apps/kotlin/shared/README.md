...
- `platformModule()` — 各 OS の Koin 束縛。`SecureKeyValueStore`（Keychain / EncryptedSharedPreferences）に加え、ダイアリー用の SQLDelight `SqlDriver` と `NetworkMonitor`（Android=`ConnectivityManager` / iOS=`NWPathMonitor`）を提供する
